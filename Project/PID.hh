...
class CPID {
    ...
    bool anti_windup, feedback; // Controller flags
    ...
    double const ulow = 0, uhigh = 4095; // Control signal limits
...
inline CPID::CPID(float _h, float _K, float b_, float Ti_, float Td_, float Tt_, float N_) {
    h = _h; K = _K; b = b_; Ti = Ti_; Td = Td_; Tt = Tt_; N = N_; P = 0; I = 0; D = 0; y_old = 0; k_old = 0; b_old = 0, ad = 0; bd = 0; ao = 0; anti_windup = true; feedback = true;
}
...
// Member function to set anti-windup flag
void CPID::set_anti_windup(bool _anti_windup) {
    anti_windup = _anti_windup;
}

// Member function to get anti-windup flag
bool CPID::get_anti_windup() {
    return anti_windup;
}

// Member function to set feedback flag
void CPID::set_feedback(bool _feedback) {
    feedback = _feedback;
}

// Member function to get feedback flag
bool CPID::get_feedback() {
    return feedback;
}
...
float CPID::compute_control(float r, float y) {
    ...
    else { //No feedback (only feedforward control signal)
        u = saturate(K*(b*r-y), ulow, uhigh);
        v = u;
    } 
